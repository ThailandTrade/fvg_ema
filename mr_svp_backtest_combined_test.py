...
# =============================================================================
SHOW_LAST_TRADES = True      # Activer/désactiver l'affichage détaillé
...
def get_session_hours(sessions: dict) -> list:
    hours = []
    for sess_name, is_active in sessions.items():
        if is_active and sess_name in SESSIONS_CONFIG:
            cfg = SESSIONS_CONFIG[sess_name]
            start_h = int(cfg['start'])
            end_h = int(cfg['end'])
            hours.extend(range(start_h, end_h))
    return list(set(hours)) if hours else list(range(24))
...
def load_all_data(conn, asset):
    ...
    active_hours = get_session_hours(sessions)
    hours_str = ','.join(str(h) for h in active_hours)
    ...
    df_candles = df_candles[df_candles['dt'].dt.hour.isin(active_hours)].reset_index(drop=True)
    if df_candles.empty:
        return df_candles, pd.DataFrame()
    ...
    query_ticks = f"""SELECT time, last as price, volume FROM {asset['tick_table']} 
        ...
        AND EXTRACT(HOUR FROM time) IN ({hours_str}) ORDER BY time ASC"""
...
def run_backtest():
    ...
    for row in df_all_candles.itertuples():
        ...
        row_week_start = (row.dt - timedelta(days=row.dt.weekday())).date()  # Lundi de la semaine
        row_week = row_week_start.strftime('%Y-%m-%d')  # Format: 2025-01-06
        ...
        if active_trade:
            ...
            if res:
                ...
                # Mise à jour des high water marks et drawdowns
