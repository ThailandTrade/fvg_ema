...
def run_backtest():
    ...
    for row in df_candles.itertuples():
        ...
        if active_trade:
            ...
            if res:
                # --- CALCUL MONEY MANAGEMENT (MODIFIÉ) ---
                risk_amount = current_capital * RISK_PERCENT # Risque basé sur capital actuel
                
                if "WIN" in res:
                    pnl = risk_amount * TARGET_RR
                    current_capital += pnl
                    stats_wins += 1
                else:
                    pnl = -risk_amount
                ...
                # --- AFFICHAGE MODIFIÉ (CAPITAL) ---
                print(f"   >>> RÉSULTAT TRADE : {res} (Sortie à {row.close}) | Cap: ${current_capital:,.2f} | MaxDD: -${max_dd_amount:.2f} | WR: {curr_wr:.2f}%")
    ...
    # Calcul du return total en %
    total_return_pct = ((current_capital - INITIAL_CAPITAL) / INITIAL_CAPITAL) * 100
    print(f"{'RESULTATS':<15} | Capital Fin: ${current_capital:,.2f} ({total_return_pct:+.2f}%) | Max Drawdown: -${max_dd_amount:.2f}")
    print("="*90)
