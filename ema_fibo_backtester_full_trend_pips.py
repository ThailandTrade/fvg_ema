...
# ---------- FONCTIONS D'AFFICHAGE (Inchangées + Nouvelle) ----------
...
def display_monthly_breakdown(all_trades_log: Dict[str, List[Dict[str, Any]]], risk_per_trade: Decimal):
    """
    Affiche un tableau des performances agrégées par mois en % avec détail LONG/SHORT.
    """
    # 1. Aplatir tous les trades en une seule liste
    all_trades = []
    ...
    # 3. Grouper par mois
    monthly_stats = {} # Key: (Year, Month), Value: Stats Dict
    
    for t in all_trades:
        dt = datetime.fromtimestamp(t['entry_time'] / 1000, tz=UTC)
        ...
        if key not in monthly_stats:
            monthly_stats[key] = {
                ...
                'pnl_r': Decimal(0),
                'running_balance': Decimal(0), # Pour calcul DD intra-mensuel
        ...
        if side == "LONG":
            stats['long_total'] += 1
            if res == "WIN": stats['long_wins'] += 1
        elif side == "SHORT":
            stats['short_total'] += 1
            if res == "WIN": stats['short_wins'] += 1
        
        # --- PnL & Drawdown ---
        stats['pnl_r'] += pnl
        
        # Calcul du Drawdown spécifique à ce mois (on remet le compteur à 0 chaque mois)
        stats['running_balance'] += pnl
        if stats['running_balance'] > stats['peak_balance']:
            stats['peak_balance'] = stats['running_balance']
        
        current_dd = stats['peak_balance'] - stats['running_balance']
        if current_dd > stats['max_dd_r']:
            stats['max_dd_r'] = current_dd

    # 4. Affichage
    print("\n" + "="*105)
    print(" 📅 MONTHLY BREAKDOWN (PORTFOLIO) - VALUES IN %")
    print("="*105)
    # On élargit un peu les colonnes pour faire tenir les infos
    print("| {:^10} | {:^8} | {:^14} | {:^14} | {:^12} | {:^12} |".format(
        "MONTH", "TRADES", "LONG WR", "SHORT WR", "PNL (%)", "MAX DD (%)"
    ))
    print("|" + "-"*12 + "|" + "-"*10 + "|" + "-"*16 + "|" + "-"*16 + "|" + "-"*14 + "|" + "-"*14 + "|")
    
    sorted_keys = sorted(monthly_stats.keys())
    
    # Facteur de conversion R vers % (ex: 0.003 -> 0.3%)
    risk_pct_factor = risk_per_trade * Decimal("100")
    ...
    for year, month in sorted_keys:
        ...
        # Calcul Long WR
        l_tot = stats['long_total']
        l_wr = (stats['long_wins'] / l_tot * 100) if l_tot > 0 else 0.0
        l_str = f"{l_wr:.1f}% ({l_tot})" if l_tot > 0 else "-"
        
        # Calcul Short WR
        s_tot = stats['short_total']
        s_wr = (stats['short_wins'] / s_tot * 100) if s_tot > 0 else 0.0
        s_str = f"{s_wr:.1f}% ({s_tot})" if s_tot > 0 else "-"
        
        # Conversion PnL et DD en %
        pnl_pct = stats['pnl_r'] * risk_pct_factor
        max_dd_pct = stats['max_dd_r'] * risk_pct_factor
        
        pnl_str = f"{float(pnl_pct):+.2f}%"
        dd_str = f"-{float(max_dd_pct):.2f}%"
        
        print("| {:<10} | {:^8} | {:^14} | {:^14} | {:>12} | {:>12} |".format(
            date_str, total, l_str, s_str, pnl_str, dd_str
        ))
...
def main():
    ...
    display_monthly_breakdown(all_trades_log, args.risk) # <--- Nouvelle version avec le risque
